...
for k in range(11) : 
    ...
    _, T_Partition = Anon_date.Anon_hour_density(df)
    Anon_date.Anon_days_density(df,T_Partition)
    Anon_pos.Anon_pos(df,"lon")
    Anon_pos.Anon_pos(df,"lat")
    Anon_Id.ids_density_anon(df)
...
def cleaning(df) : 
    ...
    project.deleteClean(df)

cleaning(final_df)

final_df.to_csv("./data/Anon_Louis", header= False, index= False, sep = "\t")  # The output files IS NOT a csv, it is separated by tabulations 
